...
//        if (true) throw new RuntimeException("FlutterBluetoothSerial Attached to activity");
        ...
        binding.addRequestPermissionsResultListener(
                (requestCode, permissions, grantResults) -> {
                    ...
                    PendingPermissionRequest completedRequest = activePermissionRequest;
                    activePermissionRequest = null;
                    completedRequest.callback.onResult(
                            hasPermissions(completedRequest.permissions));
    ...
    private static class PendingPermissionRequest {
        ...
        final EnsurePermissionsCallback callback;

        PendingPermissionRequest(
                String[] permissions,
                EnsurePermissionsCallback callback) {
            this.permissions = permissions;
            this.callback = callback;
        }
    }

    private final List<PendingPermissionRequest> permissionRequestQueue =
            new ArrayList<>();
    private PendingPermissionRequest activePermissionRequest = null;

    private String[] discoveryPermissions() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            // Device names and addresses require CONNECT while discovery itself
            // requires SCAN on Android 12 and newer.
            return new String[]{
        ...
        // With UDPCore's target SDK, classic discovery needs fine location on
        // Android 6 through Android 11. It is capped out of newer manifests.
        return new String[]{Manifest.permission.ACCESS_FINE_LOCATION};
    }

    private String[] connectPermissions() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
    ...
    private String[] missingPermissions(String[] permissions) {
    ...
    private void ensurePermissions(
        ...
        permissionRequestQueue.add(
                new PendingPermissionRequest(permissions, callback));
        processNextPermissionRequest();
    }

    private void processNextPermissionRequest() {
        if (activity == null || activePermissionRequest != null
                || permissionRequestQueue.isEmpty()) {
            return;
        }

        PendingPermissionRequest request = permissionRequestQueue.remove(0);
        String[] missing = missingPermissions(request.permissions);
        if (missing.length == 0) {
            request.callback.onResult(true);
            processNextPermissionRequest();
            return;
        }
    ...
    private void ensureDiscoveryPermissions(EnsurePermissionsCallback callback) {
        ensurePermissions(discoveryPermissions(), callback);
    }

    private void ensureConnectPermissions(EnsurePermissionsCallback callback) {
        ensurePermissions(connectPermissions(), callback);
    }
    ...
    private void connectToAddress(String address, Result result) {
    ...
    private class FlutterBluetoothSerialMethodCallHandler implements MethodCallHandler {
        ...
        public void onMethodCall(MethodCall call, Result result) {
            ...
            switch (call.method) {
                ...
                case "requestDiscoveryPermissions":
                    ensureDiscoveryPermissions(result::success);
                    break;

                case "requestConnectPermissions":
                    ensureConnectPermissions(result::success);
                    break;
                ...
                case "getBondedDevices":
                    ensureConnectPermissions(granted -> {
                        if (!granted) {
                            result.error(
                                    "no_permissions",
                                    "Nearby Devices permission is required to list bonded devices",
                                    null);
                            return;
                        }
                ...
                case "startDiscovery":
                    ensureDiscoveryPermissions(granted -> {
                        if (!granted) {
                            result.error(
                                    "no_permissions",
                                    Build.VERSION.SDK_INT >= Build.VERSION_CODES.S
                                            ? "Nearby Devices permission is required for Bluetooth discovery"
                                            : "Location permission is required for Bluetooth discovery on Android 11 and older",
                                    null);
                            return;
                        }
                ...
                case "connect": {
                    ...
                    ensureConnectPermissions(granted -> {
                        ...
                        connectToAddress(targetAddress, result);
                    });
                    break;
                }
