...
class FlutterBluetoothSerial {
  ...
  /// Requests only the permissions needed to list or connect to devices.
  ///
  /// Android 12 and newer request Nearby Devices (`BLUETOOTH_CONNECT`).
  /// Android 11 and older need no runtime permission for an existing bond.
  Future<bool> requestConnectPermissions() async =>
      await _methodChannel.invokeMethod('requestConnectPermissions') ?? false;

  /// Requests only the permissions needed for classic Bluetooth discovery.
  ///
  /// Android 12 and newer request Nearby Devices (`BLUETOOTH_SCAN` and
  /// `BLUETOOTH_CONNECT`). Android 11 and older request location, as required
  /// by the platform's legacy discovery API.
  Future<bool> requestDiscoveryPermissions() async =>
      await _methodChannel.invokeMethod('requestDiscoveryPermissions') ?? false;
